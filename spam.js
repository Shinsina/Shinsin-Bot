...
client.on("message", (msg) => {
  const containsDiscordUrl = msg.content
    .toString()
    .match(/discord\.gg\/\w*\d*/);
  const containsDiscordAltUrl = msg.content
    .toString()
    .match(/discord\.com\/invite\/\w*\d*/);
  const containsTwitchUrl = msg.content.toString().match(/twitch\.tv\/\w*\d*/);
  const containsPinkDiscord = msg.content
    .toString()
    .match(/discord\.gg\/pink$/);
  ...
  if (msg.member.user.tag != "Shinsin-Bot#1849") {
    if (whitelist.indexOf(msg.member.user.tag) == -1 && msg.member.bannable) {
      ...
      if (getAllIndices(workingList, msg.member.user.tag).length >= 8) {
        client.channels.cache
          .get("797233265572053022")
          .send(msg.member.user.tag + " has been banned for spam");
      ...
      if (
        containsDiscordUrl != null ||
        containsDiscordAltUrl != null ||
        containsTwitchUrl != null
      ) {
        if (containsPinkDiscord != null) {
          msg.reply(
            "Check the pinned messages in the general channel. Lee los mensajes 'pinned' en el canal general"
          );
        } else {
          client.channels.cache
            .get("797233265572053022")
            .send(msg.member.user.tag + " has been banned for self-promotion");
    ...
    if (msg.content == "!shinsin-bot") {
      msg.channel.send(
        "Shinsin-Bot is a Discord bot developed by Shinsina, used as a moderation tool."
      );
    }
  }
});
